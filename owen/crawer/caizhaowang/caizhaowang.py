...
ws.column_dimensions['E'].width = 52.38
ws.append(["省份", "招标信息", "关键字", "日期", "招标网址"])
    ...
    for k in soup.find_all(id="searchcontent"):
        ...
        for li in k.find_all('li'):
            for left in li.find_all('div', class_="s_c_l_left"):
                ...
                keyWord = config.findKeyWord( config.fix(a[1].string) )
                # 如果不存在关键字
                if keyWord == False:
                    continue
                # 省份，信息，关键字，日期, 网址
                line = [config.fix(a[0].string), config.fix(a[1].string), keyWord, config.fix(right[0].get_text()), "https://www.bidcenter.com.cn" + a[1]['href']]
                # 写入
                ws.append(line)
